...
def semesterlist(request):
  semesters = list(
    Semester.objects.filter(school=request.session['school']).order_by('-id')[
    :])
  ...
  if request.method == 'POST' and 'feeupdate' in request.POST:
    ...
    for kk in request.POST:
      ...
      try:
        ...
        if key not in ['basecc', 'basechk', 'book', 'material', 'misc',
                       'mdiscount']:
          continue
        fid = id_decode(fid)
        if fid in fees:
...
def create_default_grading_categories(theClass):
  order = 1
  for name, weight, assignment in [('Quiz', 30, False), ('Exam', 40, False),
                                   ('Home Work', 30, True),
                                   ('Other', 0, False)]:
...
def family_tuition(request, fid):
  ...
  semester = current_record_semester(
    request)  # the semester open for registration
  tuition, created = Tuition.objects.get_or_create(family=family,
                                                   semester=semester)
...
def payment(request, filter=None):
  semester = current_record_semester(
    request.session['school'])  # the semester open for registration
  # if not semester:
  ...
  if request.method == 'POST':
    ...
    if 'domain_type' not in request.POST:
      ...
      if valid_domain(domain):
        request.session['school'], _c = School.objects.get_or_create(
          domain=domain)
        print 'domain', domain
