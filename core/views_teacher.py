...
def grading_policy(request, cid):
  ...
  if permitted:
    ...
    if request.method == 'POST':
      keys = [tuple(k.split('-') + [request.POST[k]]) for k in request.POST if
              '-' in k]
  ...
  if request.method == 'GET':
    ...
    for k in score_keys:
      try:
        ...
        score, created = Score.objects.get_or_create(gradingItem=gi,
                                                     student=student)
        score.score = ss
        score.save()
      except Exception, e:
...
def del_attendance(request, cs_id):
  ...
  return HttpResponseRedirect(
    '/record_attendance/%s' % (id_encode(theClass.id),))
...
def edit_attendance(request, cs_id):
  ...
  try:
    cs.date = date = datetime.datetime.strptime(request.POST['date'],
                                                '%m-%d-%y')
    cs.save()
  except Exception, e:
    print e
...
def assignments(request, cat_id):
  ...
  if request.method == 'POST':
    name = request.POST['Name']
    gi, created = GradingItem.objects.get_or_create(category=category,
                                                    name=name)
  ...
  print due
  assignment_date = DateField().widget.render('assign_date',
                                              datetime.datetime.today(),
                                              attrs={'id': 'assign_date'})
  due_date = DateField().widget.render('due_date', due,
                                       attrs={'id': 'due_date'})
...
def edit_assignment(request, aid):
  ...
  if request.method == 'POST':
    if 'Delete' in request.POST:
      ...
      # name=request.POST['Name']
      gi.date = None  # request.POST['assign_date']
      gi.duedate = datetime.datetime.strptime(request.POST['due_date'],
                                              '%m-%d-%y')
      gi.assignmentDescr = request.POST['Description']
  ...
  due_date = DateField().widget.render('due_date', gi.duedate,
                                       attrs={'id': 'due_date'})
...
def get_attendance(student, session):
  try:
    return att_dict[
      Attendance.objects.get(student=student, session=session).attended]
  except:
    return att_dict['P']
...
def report_card(request, enrolldetail_id):
  ...
  attendances = [(session.date, get_attendance(student, session)) for session in
                 sessions]
  ...
  for cat in categories:
    ...
    if not scores:
      ...
      cat.avg_score = sum(
        [(score[1] if score[1] != None else 0) for score in scores]) / len(
        scores)
    try:
      cat.class_avg = sum(
        [(item.average if item.average != None else 0) for item in
         items]) / len(scores)
    except:
      cat.class_avg = 0
...
def notify_report_card(request, class_id):
  ...
  for en in ens:
    ...
    en.student.family.user.email_user(subject, message,
                                      settings.DEFAULT_FROM_EMAIL)
  return generic_message(request, 'Parents Notified', 'teacher',
                         'Parents have been notified that report cards are ready.')
